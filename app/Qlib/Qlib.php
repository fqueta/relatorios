...
class Qlib
{
  ...
  static function totalReg($tabela, $condicao = false,$debug=false){
    ...
    static function somarCampo($tabela, $campo_ancora, $condicao_busca = false, $debug = false){
		//necessário quando trabalhamos com eZsql db class
		$sql = "SELECT SUM(" . $campo_ancora . ") AS totalreg FROM {$tabela} $condicao_busca";
		if($debug)
			echo $sql;
		$td_registros = DB::select($sql);;
		if(isset($td_registros[0]->totalreg) && $td_registros[0]->totalreg > 0){
  ...
  static function decodeArray($arr){
    ...
    static function CalcularMeses($data,$meses,$sentido = '-',$retDiaUtl=true)
	{
		// $sentido - para tras sentido + para frente
        $formato = 'd/m/Y';
		...
		if ($meses==0)
		{
			...
			return Qlib::proximoDiaUtil(Qlib::dtBanco($data1), $formato);
		}
		else
		{
			if($sentido=='-'){
